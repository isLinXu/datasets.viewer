...
class DatasetViewer:
    ...
    def load_sidebar(self):
        with st.sidebar:
            ...
            # 新增的图片标记功能
            self.image_tag = st.selectbox("标记当前图片:", ["", "bad", "good"])
            if self.image_tag and self.image_folder_path:
                images = get_files(self.image_folder_path, [".jpg", ".png", ".jpeg", ".bmp", ".tiff"])
                if images:
                    image_file = images[self.image_index]
                    if self.image_tag == "bad":
                        self.bad_images.append(image_file)
                        st.sidebar.success(f"已标记为bad的图片：{self.bad_images}")
                    elif self.image_tag == "good":
                        self.good_images.append(image_file)
                        st.sidebar.success(f"已标记为good的图片：{self.good_images}")
            if st.sidebar.button("删除bad的图片"):
                for image_file in self.bad_images:
                    image_path = os.path.join(self.image_folder_path, image_file)
                    if os.path.exists(image_path):
                        os.remove(image_path)
                        st.sidebar.success(f"已删除图片：{image_path}")
            if st.sidebar.button("导出保存good的图片"):
                save_path = os.path.join(self.image_folder_path, "good_images")
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                for image_file in self.good_images:
                    image_path = os.path.join(self.image_folder_path, image_file)
                    if os.path.exists(image_path):
                        shutil.copy(image_path, save_path)
                        st.sidebar.success(f"已保存图片到：{os.path.join(save_path, image_file)}")
